...
void thread_set_priority(int new_priority)
{
  ...
  old_level = intr_disable();
  if (thread_current()->base_priority == thread_current()->priority || new_priority > thread_current()->priority)
...
void thread_lock_will_wait(struct lock *lock)
{
  ...
  while (thread != NULL && nesting < PRI_MAX_DONATION_NESTING)
  {
    ...
    {
      if (thread->status == THREAD_READY && maybe_raise_priority(thread, thread_current()->priority))
...
void thread_lock_released(struct lock *lock)
{
  ...
  for (e = list_begin(locks_owned_list); e != list_end(locks_owned_list);)
  {
    ...
    else
    {
      ...
      if (waiting_thread != NULL && waiting_thread->priority > highest_waiting_priority)
...
update_priority(struct thread *t)
{
  ...
  t->priority = trim_priority(FP_TO_INT(INT_TO_FP(PRI_MAX) - t->recent_cpu / 4 - INT_TO_FP(t->nice * 2)));
}
